...
// 脚本      
const ab = document.getElementById('itemall')
ab.onclick=function(evt){
    console.dir(ab.children)// 取这个值
    console.log(evt.target)// 取这个值
    for (let index = 0; index < ab.children.length; index++) {
        const element = ab.children[index];
        if(element===evt.target){// 做比较
